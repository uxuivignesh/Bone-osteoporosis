function [out1,out2] = neurofuzzy(varargin)
...
function [w1,b1,w2,b2,tr] = designrb(p,t,eg,sp,mn,df)
  ...
  tr.perf(2) = MSE;
  if isfinite(df)
    fprintf('NEUROFUZZY, neurons = 0, MSE = %g\n',tr.perf(1));
  end
  flag_stop=plotperfrb(tr,eg,'NEUROFUZZY',0);
  ...
  for k = 2:iterations
    ...
    if isfinite(df) & (~rem(k,df))
      fprintf('NEUROFUZZY, neurons = %g, MSE = %g\n',k,MSE);
      error=error+MSE;
      accuracy= 100-error;
      disp('FEATURE CLASSIFICATIOIN_NEUROFUZZY_ ERROR');
      disp(error);
      disp('FEATURE CLASSIFICATIOIN_NEUROFUZZY_ ACCURACY');
      disp(accuracy);
      flag_stop=plotperfrb(tr,eg,'NEUROFUZZY',k);
    end
